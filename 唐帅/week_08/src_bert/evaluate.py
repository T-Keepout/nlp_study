...
def plot_similarity_distribution(
    sims, labels, threshold, save_path, title="相似度分布"
):
    """
...
def parse_args():
    ...
    parser.add_argument(
        "--model_type", required=True, choices=["biencoder", "crossencoder"]
    )
    parser.add_argument(
        "--ckpt", required=True, type=str, help="checkpoint .pt 文件路径"
    )
    ...
    parser.add_argument(
        "--max_length", default=64, type=int, help="BiEncoder 单句最大长度"
    )
    parser.add_argument(
        "--split",
        default="validation",
