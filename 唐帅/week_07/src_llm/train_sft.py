...
DATA_DIR = EXP_ROOT / "data" / "peoples_daily"
MODEL_PATH = EXP_ROOT / "pretrain_models" / "Qwen2-0.5B-Instruct"
...
class SFTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        response_ids = self.tokenizer.encode(target, add_special_tokens=False) + [
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="LLM SFT NER 训练（人民日报数据集，LoRA / 全量）"
    )
...
def main():
    ...
    if args.full_ft:
        ...
        print(
            f"trainable params: {total:,} || all params: {total:,} || trainable%: 100.0000"
        )
    else:
    ...
    print(
        f"总训练步数: {total_steps}（batch={args.batch_size}, "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if avg_val_loss < best_val_loss:
            ...
            print(
                f"  ✓ 最优{ckpt_label}已保存 → {ckpt_dir}  (val_loss={avg_val_loss:.4f})"
            )
