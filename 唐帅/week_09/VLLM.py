...
# ── 配置 ──────────────────────────────────────────────────────────────
MODEL_PATH = "/home/keepout/AI/LLM/作业/week_09/pretrain_models/Qwen2-0.5B-Instruct"
N_PROMPTS = 50
MAX_NEW_TOKENS = 100
BATCH_SIZE = 8
...
def bench_transformers(prompts: list[str]) -> dict:
    ...
    for i in range(0, len(chat_prompts), BATCH_SIZE):
        ...
        print(
            f"    进度 batch {i//BATCH_SIZE + 1}/{(len(chat_prompts)+BATCH_SIZE-1)//BATCH_SIZE}"
        )
    dt_b = time.time() - t0
    ...
    return {
        "serial": {
            "time": dt_a,
            "gen_tokens": total_tokens_a,
            "qps": len(prompts) / dt_a,
            "tps": total_tokens_a / dt_a,
        },
        "batch": {
            "time": dt_b,
            "gen_tokens": total_tokens_b,
...
def plot_results(r: dict, out_path: str):
    ...
    for b, v in zip(bars, qps):
        axes[1].text(
            b.get_x() + b.get_width() / 2, v, f"{v:.1f}", ha="center", va="bottom"
        )
...
def main():
    print("=" * 70)
    print(
        f"  Throughput Benchmark  |  {N_PROMPTS} prompts × max {MAX_NEW_TOKENS} new tokens"
    )
    print("=" * 70)
    ...
    print("  核心结论：")
    print(
        f"    vLLM 相对 transformers 串行加速：{results['vllm']['qps']/results['serial']['qps']:.1f}×"
    )
    print(
        f"    vLLM 相对 transformers batch:    {results['vllm']['qps']/results['batch']['qps']:.1f}×"
    )
