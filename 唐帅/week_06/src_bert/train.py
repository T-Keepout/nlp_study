...
def parse_args():
    ...
    parser.add_argument(
        "--warmup_ratio", default=0.1, type=float, help="warmup 步数占总步数的比例"
    )
    parser.add_argument(
        "--grad_accum", default=1, type=int, help="梯度累积步数，显存不足时设为 2/4"
    )
    parser.add_argument(
        "--use_class_weight",
        action="store_true",
...
def train_one_epoch(
    ...
    for step, batch in enumerate(pbar):
        ...
        # 累计梯度更新，相当于(loss1 + loss2 +...+ loss_grad_accum)/grad_accum
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        val_metrics = evaluate_model(
            model, val_loader, device, id2name, print_report=(epoch == args.epochs)
        )
